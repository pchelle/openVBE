...
plt <- ggplot() + geom_ribbon(data = mmdf[mmdf$drug == "Petrolatum",],
                              mapping = aes(x = time/60,
                                            y = (median)*1e5,
                                            ymin = (min)*1e5,
                                            ymax = (max)*1e5,
                                            fill = drug),alpha = 0.25) + theme_minimal()

pkData <- read.csv(pkDataFilePath)
pkData$drug <- "Petrolatum"
pkData$id <- as.factor(pkData$id)
plt <- plt + geom_point(data = pkData, mapping = aes(x = time,y = (outputValues)*1e5,color = id,
                                                     shape = "Individual"))

plt <- plt + xlab("Time (h)") + ylab(expression(Testosterone~plasma~concentration%*%10^5~(µg/l)))
plt <- plt + guides(color = "none")
plt <- plt + theme(panel.background = element_rect(fill = "white", colour = "white"),
...
vbeplt <- vbeplt + theme(panel.background = element_rect(fill = "white", colour = "white"),
                         plot.background = element_rect(fill = "white", colour = "white"),
                         legend.position = "bottom", legend.direction = "horizontal",
                         axis.text = element_text(size = 12),axis.title = element_text(size = 14),
                         legend.text = element_text(size = 12),legend.title = element_text(size = 14),
