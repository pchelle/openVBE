...
for (fn in names(fnVec)){
  df <- aggregate(x = refAndTestSimulationsInVirtualPopulation$conc,
                  by = list(refAndTestSimulationsInVirtualPopulation$time,
...
plt <- ggplot() + geom_ribbon(data = mmdf[mmdf$drug == "R", ],
                              mapping = aes(x = time/60,
                                            y = (median),
                                            ymin = (min),
                                            ymax = (max),
                                            fill = drug),
                              alpha = 0.25) + theme_minimal()
plt <- plt + geom_point(data = na.omit(pkData), mapping = aes(x = time/60,
                                                              y = (outputValues),
                                                              color = id,
                                                              shape = "Individual"))
...
Tplt <- ggplot() + geom_ribbon(data = mmdf[mmdf$drug == "T1", ],
                               mapping = aes(x = time/60,
                                             y = (median),
                                             ymin = (min),
                                             ymax = (max),
...
Tplt <- Tplt + theme(panel.background = element_rect(fill = "white", colour = "white"),
